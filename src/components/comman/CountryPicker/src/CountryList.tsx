...
const CountryItem = (props: CountryItemProps) => {
  ...
  return (
    country?.callingCode.length > 0 && <TouchableOpacity
      key={country.cca2}
      testID={`country-selector-${country.cca2}`}
      onPress={() => onSelect(country)}
      {...{ activeOpacity }}
    >
      <View style={[styles.itemCountry, commonStyle.flexDirection(lang), { height: itemHeight }]}>
        {withFlag && (
          <Flag
            {...{ withEmoji, countryCode: country.cca2, flagSize: flagSize! }}
          />
        )}
        <View style={[styles.itemCountryName, commonStyle.alignItems(lang)]}>
